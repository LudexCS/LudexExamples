...
async function functionBody (attachment: APIAttachment, req: Request, res: Response) {
    ...
    const seller: bigint = BigInt(data.seller);
    ...
    const itemRegistry = facade.readonlyAccessItemRegistry();
    ...
    const sellerProxy = facade.serviceAccessSellerProxy();

    const [itemID, sharerIDs] = 
        await sellerProxy.registerItem(
            nameHash,
            seller,
            sharers,
            itemPrice,
...
export function postAPIDelegateRegister()
{return (attachment: APIAttachment) => {
    attachment.app.post("/api/delegate-register", async (req: Request, res: Response) => {
